        ...
        if div_val == 1:
            ...
            for i in range(len(cutoff_ends) - 1):
                with tf.compat.v1.variable_scope('cutoff_{}'.format(i)):
                    ...
                    mask_idx = tf.cast(tf.where(mask), tf.int64)
                    # y += tf.scatter_nd(mask_idx, cur_y, tf.to_int64(tf.shape(y)))
                    y += tf.scatter_nd(mask_idx, cur_y, tf.cast(tf.shape(y), tf.int64))
        ...
        if len(cutoffs) == 0:
            ...
            for i in range(len(cutoff_ends) - 1):
                with tf.compat.v1.variable_scope('cutoff_{}'.format(i)):
                    ...
                    nll += tf.scatter_nd(mask_idx, -cur_logprob, tf.cast(tf.shape(nll), tf.int64))
    if return_mean:
...
def transformer(dec_inp, target, mems, n_token, n_layer, d_model, d_embed,
                ...
                initializer, is_training, generate_text, prev_text, proj_initializer=None,
  ...
  """
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        if generate_text:
            qlen = tf.shape(prev_text)[0]
            input_data = prev_text
        else:
        ...
        if generate_text == False:
            ...
            params_W, params_projs = shared_params[0], shared_params[1]
            softmax_b = tf.zeros([n_token])
            output = _logit(output, params_W, softmax_b, params_projs)
